...
func TestGetBorrowTimeRange(t *testing.T) {
	btr := types.GetBorrowTimeRange()

	t.Run("one week", func(t *testing.T) {
		r, err := getBorrowTimeRange(btr.OneWeek)
		assert.NoError(t, err)
		assert.Equal(t, 7, r)
	})

	t.Run("two week", func(t *testing.T) {
		r, err := getBorrowTimeRange(btr.TwoWeek)
		assert.NoError(t, err)
		assert.Equal(t, 14, r)
	})

	t.Run("one month", func(t *testing.T) {
		r, err := getBorrowTimeRange(btr.OneMonth)
		assert.NoError(t, err)
		assert.Equal(t, 30, r)
	})

	t.Run("two month", func(t *testing.T) {
		r, err := getBorrowTimeRange(btr.TwoMonth)
		assert.NoError(t, err)
		assert.Equal(t, 60, r)
	})

	t.Run("custom time 120", func(t *testing.T) {
		r, err := getBorrowTimeRange("120")
		assert.NoError(t, err)
		assert.Equal(t, 120, r)
	})

	t.Run("not a number", func(t *testing.T) {
		r, err := getBorrowTimeRange("abcdefg")
		assert.Error(t, err)
		assert.Equal(t, 0, r)
	})
}
